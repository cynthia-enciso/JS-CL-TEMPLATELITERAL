...
test('should change e1 dom node text to be correct name', () => {
    exercise1("Bobby");
    expect(document.getElementById("e1").innerText.toLowerCase().trim()).toBe("hey bobby,");
});

test('should change e2 dom node text to include correct values', () => {
    exercise2("grand", "read books", "fabulous");
    expect(document.getElementById("e2").innerText.toLowerCase().trim())
...
test('should change e3 dom node text to be correct', () => {
    exercise3("fantastic");
    expect(document.getElementById("e3").innerText.trim()).toBe("I hope you're having a FaNtAsTiC day!");
});

// template literal checks
test('source code should contain exercise 1 template literals',() => {
    // Read the content of the source file
    const sourceCode = fs.readFileSync('./src/index.js', 'utf8').toLowerCase();
